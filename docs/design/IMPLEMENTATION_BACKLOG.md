...
## Epic 4 — Analysis
- [x] dependency graph — `analysis/dependencies.py` (pyproject.toml/requirements.txt, package.json, Cargo.toml manifests parsed; go.mod/pom.xml/build.gradle/Gemfile detected as ecosystem evidence only, not yet parsed) + `analysis/relationships.py` (DEPENDS_ON, now Component-level, not just Repository-level)
