...
- [x] Proposal -> ChangePlan (deterministic case only) — `proposals/engine.py::change_plan_for_component_update`; a `pypi` dependency whose current constraint is confirmed as an exact pin can be turned into an executable `ChangePlan` with no human/LLM authoring a diff by hand (`si check-updates --plan-out DIR` -> `si execute <file> <target> --approve`). Every other Proposal kind (creation/adoption/integration, and npm/range-constrained updates) still has no automatic path to a ChangePlan — that remains a job for a human or an external implementer (e.g. Claude Code), never guessed here (ADR-007)
