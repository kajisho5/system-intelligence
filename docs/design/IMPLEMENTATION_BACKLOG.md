...
## Epic 3 — Discovery
- [x] filesystem scanner — `discovery/structure.py`, backed by `discovery/paths.py::EXCLUDED_DIRS` (`vendor` — Go's `go mod vendor`/PHP Composer's own dependency-vendoring convention, a full copy of every dependency's own source tree including its own manifest — plus `.tox`/`target`, the same "dependency cache/build output" shape already covered for other ecosystems); a vendored dependency's own go.mod/package.json/etc. was previously discovered as if it were the target repository's own, misattributing its transitive dependencies
...
- [x] package detector — `discovery/structure.py`'s `PACKAGE_MANIFESTS` (includes `Pipfile` for Pipenv-only Python projects and `build.gradle.kts` for Gradle's Kotlin DSL, alongside each ecosystem's other already-listed manifest filenames)
