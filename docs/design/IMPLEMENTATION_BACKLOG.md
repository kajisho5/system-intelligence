...
## Epic 4 — Analysis
- [x] dependency graph — `analysis/dependencies.py` (pyproject.toml/requirements.txt, package.json, Cargo.toml, and go.mod manifests parsed; pom.xml/build.gradle/Gemfile detected as ecosystem evidence only, not yet parsed — each has its own hidden complexity, e.g. pom.xml's `${property}` version placeholders needing possibly-cross-file resolution, that a direct port of the existing pattern would get wrong) + `analysis/relationships.py` (DEPENDS_ON, now Component-level, not just Repository-level)
