...
| `si check-updates <target>` | Component Update Intelligence: current vs. available state (network); `--check-vulnerabilities` also looks up known vulnerabilities (OSV.dev) for each resolved version; `--plan-out DIR` writes a ready-to-run `ChangePlan` for a pypi or npm exact-pin bump, no hand-written JSON needed |
