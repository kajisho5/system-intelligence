...
Only `si doctor` and `si version` are implemented in Phase 1. The remaining
commands from docs/design/docs/13-cli-and-ux.md (`inspect`, `diagnose`,
`research`, `design`, `improve`, `propose`, `execute`, `verify`, `report`,
`diff`, `watch`) are registered as explicit placeholders so `si --help`
documents the intended surface without claiming functionality that does not
exist yet.
"""
...
_PLANNED_COMMANDS = {
    "inspect": "Read-only inventory of a target. Planned for Phase 2 (discovery).",
